...
def publish():
    ...
    local("echo $'User-agent: *\nSitemap: https://blog.burdz.net/sitemap.xml' > {deploy_path}/robots.txt".format(**env))
    local("ghp-import -b {github_pages_branch} {deploy_path}".format(**env))
    local("git checkout {github_pages_branch}".format(**env))
    local('git commit --amend -m "beep boop - publishing changes to blog.burdz.net - [ci skip]"'.format(**env))
